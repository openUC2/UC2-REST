...
#define ISR_MIN_INTERVAL 5  // for 1ms, adjust as needed
...
void IRAM_ATTR capture_handler(void *arg) {
  uint32_t currentISRTime = micros();
  if ((currentISRTime - lastISRTime) < ISR_MIN_INTERVAL) {
    return;  // Exit ISR if called too soon
  }
  ...
  if (cap_data[cap_unit].period != 0) {
    ...
    if ((float)cap_data[cap_unit].period > 0){
      /*Serial.print((float)high_time );
      Serial.print("/");
      Serial.println((float)cap_data[cap_unit].period);*/
    latest_duty_cycle[cap_unit] = ((float)high_time / (float)cap_data[cap_unit].period) ;
...
void loop() {
  Serial.println(latest_duty_cycle[0] );
}
