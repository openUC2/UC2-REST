...
#if defined(ESP32) || defined(ESP8266) // ESP
...
WebThingAdapter::WebThingAdapter(ThingDevice *_thing, String _name,
                                 IPAddress _ip, uint16_t _port)
...
void WebThingAdapter::begin() {
  ...
  this->server.onNotFound(
      std::bind(&WebThingAdapter::handleUnknown, this, std::placeholders::_1));

  this->server.on(
      "/*", HTTP_OPTIONS,
      std::bind(&WebThingAdapter::handleOptions, this, std::placeholders::_1));
  ...
  while (action != nullptr) {
    ...
    this->server.on(actionBase.c_str(), HTTP_DELETE,
                    std::bind(&WebThingAdapter::handleThingActionDelete, this,
                              std::placeholders::_1, this->thing, action));
  ...
  ws->onEvent(std::bind(&WebThingAdapter::handleWS, this,
                        std::placeholders::_1, std::placeholders::_2,
                        std::placeholders::_3, std::placeholders::_4,
...
void WebThingAdapter::sendErrorMsg(DynamicJsonDocument &prop,
                                   AsyncWebSocketClient &client, int status,
                                   const char *msg) {
  prop["error"] = msg;
  prop["status"] = status;
  String jsonStr;
...
void WebThingAdapter::handleWS(AsyncWebSocket *server,
                               AsyncWebSocketClient *client, AwsEventType type,
                               void *arg, const uint8_t *rawData, size_t len,
                               ThingDevice *device) {
...
void WebThingAdapter::sendChangedProperties(ThingDevice *device) {
...
void WebThingAdapter::handleThing(AsyncWebServerRequest *request,
                                  ThingDevice *&device) {
  if (!verifyHost(request)) {
    return;
  }
...
void WebThingAdapter::handleThingEventGet(AsyncWebServerRequest *request,
                                          ThingDevice *device,
                                          ThingItem *item) {
  if (!verifyHost(request)) {
    return;
...
void WebThingAdapter::handleBody(AsyncWebServerRequest *request, uint8_t *data,
