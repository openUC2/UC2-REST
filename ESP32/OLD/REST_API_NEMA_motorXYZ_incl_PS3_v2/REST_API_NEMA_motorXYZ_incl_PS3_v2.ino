...
void setup() {
  ...
  */
  ...
  // assign a task for the motor so that it runs when we need it
  xTaskCreate(
    runsofi,             // Task function.
    "runsofi",           // String with name of task.
    1000,                   // Stack size in words.
    (void*)&glob_amplitude_sofi,      // Parameter passed as input of the task
    1,                         // Priority of the task.
    NULL);                     // Task handle.
...
void runsofi(void * parameter) {
  while (true) {
    int ampl_z = random(*((int*)parameter));
    int ampl_x = lens_x + abs(sgn(random(*((int*)parameter))) * 50);
    run_lens_z(ampl_z);
    run_lens_x(ampl_x);
    Serial.println(ampl_z);
    Serial.println(ampl_x);
    delay(50);
    run_lens_z(0);
    run_lens_x(lens_x);
    delay(50);
